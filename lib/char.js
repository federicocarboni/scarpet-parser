 ...
 * [`Character.isWhitespace(char)`][1]. This is not the same as Unicode whitespace, check the Java
 * docs for details.
 *
...
export function isJavaWhitespace(c) {
    ...
    return JAVA_WHITESPACE_CHAR_CODES.includes(c);
}
...
/**
 * Returns whether `c` is a decimal digit character (should closely match Java
...
/**
 * Returns whether `c` is a Unicode Letter character (should closely match Java
...
/**
 * Returns the numeric value of `c` (should closely match Java's [`Character.digit(char, int)`][1]
 * with `10` as radix).
 *
 * Like the reference Java method supplementary characters are not supported.
 *
 * [1]: https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/lang/Character.html#digit(char,int)
 *
 * @param {number} c - Char code (`\u0000-\uFFFF`)
 * @returns {number | undefined} Decimal digit value of `c` or `undefined` if `c` is not a decimal
 *   digit character.
 */
export function getNumericValue(c) {
    if (0x30 /* 0 */ <= c && c <= 0x39 /* 9 */) return c - 0x30;
    return c > 127 ? DECIMAL_DIGIT_CHAR_CODES.get(c) : undefined;
}
