...
export class Tokenizer {
    ...
    /**
     * @private
     * @returns {import('./Token.js').Position} Current position in `input`.
     */
    getPosition() {
     ...
     *
     * @private
     * @param {DiagnosticCode} diagnostic - Diagnostic to report.
     */
    ...
    readNumber() {
        ...
        if (c === 0x30 /* 0 */ && (this.peekChar() | 0x20) /* SP */ === 0x78 /* x */) {
            ...
            if (!isHexDigit((c = this.getChar()))) {
                this.error(DiagnosticCode.ExpectedHexDigit);
        ...
        while (this.offset < this.input.length) {
            if (isDigit(c)) {
                ...
                // tokenizer... yet...
                if (c > 127) {
                    this.error(DiagnosticCode.UnicodeDigit);
                    isUnicode = true;
                } else {
                    if (isExp) {
                        exp *= 10;
                ...
                if (intValue > Number.MAX_SAFE_INTEGER)
                    this.error(DiagnosticCode.LossOfPrecision, pos);
        ...
        while (c !== 0x27 /* ' */) {
            if (c === 0x5c /* \ */) {
                ...
                if (c === 0x6e /* n */) {
                    ...
                    this.offset--;
                    this.error(DiagnosticCode.UnknownEscapeSequence);
                }
            }
            this.offset++;
            this.character++;
    ...
    readComment() {
        ...
        if (!this.allowComments) this.error(DiagnosticCode.UnexpectedComment, pos, end);
    ...
    readEllipsis() {
        ...
        if (this.getChar() !== 0x2e /* . */ || this.peekChar() !== 0x2e) {
            this.error(DiagnosticCode.UnknownOperator, pos);
    ...
    getBooleanOp(tokenType, char) {
        ...
        if (this.peekChar() !== char) {
            this.error(DiagnosticCode.UnknownOperator);
            return this.nextToken();
        } else {
            this.offset++;
            this.character++;
    ...
    nextToken() {
        ...
        switch (c) {
            ...
            case 0x24 /* $ */:
                if (!this.allowNewLineMarkers) this.error(DiagnosticCode.UnexpectedNewLineMarker);
