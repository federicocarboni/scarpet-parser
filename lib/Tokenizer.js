import {TOKEN_NONE, TokenType, createToken} from './Token.js';
...
export class Tokenizer {
    ...
    readNumber() {
        ...
        if (c === 0x30 /* 0 */ && (this.peekChar() | 0x20) /* SP */ === 0x78 /* x */) {
            ...
            return (this.prevToken = createToken(TokenType.HexNumber, pos, end, {
                value,
                scale: 0n,
            }));
        }
        ...
        return (this.prevToken = createToken(TokenType.Number, pos, end, {value, scale}));
    }
    ...
    readString() {
        ...
        while (c !== 0x27 /* ' */) {
            if (c === 0x5c /* \ */) {
                ...
                if (this.offset >= this.input.length) {
                    ...
                    return (this.prevToken = createToken(
                        TokenType.String,
                        pos,
                        this.getPosition(),
                        value,
        ...
        value += this.input.slice(start, this.offset++);
        this.character++;
        return (this.prevToken = createToken(TokenType.String, pos, this.getPosition(), value));
    }
        ...
        const tokenType =
            this.getChar() === 0x28 /* ( */
                ...
                : constants.includes(value)
                  ? TokenType.Constant
                  : TokenType.Variable;
        return (this.prevToken = createToken(tokenType, pos, end, value));
    }
    ...
    readEllipsis() {
        ...
        return (this.prevToken = createToken(TokenType.Spread, pos, this.getPosition()));
    }
    ...
    getOneCharToken(tokenType) {
        ...
        this.offset++;
        this.character++;
        return (this.prevToken = createToken(tokenType, pos, this.getPosition()));
    }
    ...
    /**
     * @returns {import('./Token.js').Token} Reads the next token, or `Token.NONE` if end of file is
     *   reached.
     */
    nextToken() {
        this.skipWhitespace();
        if (this.offset >= this.input.length) {
            return (this.prevToken = TOKEN_NONE);
        }
