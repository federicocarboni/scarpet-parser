import {Diagnostic, DiagnosticCode} from './diagnostic.js';
 ...
 *   input.
 * @property {Diagnostic[]} [errors=[]] Default is `[]`
 * @property {Diagnostic[]} [warnings=[]] Default is `[]`
 */
...
export class Parser {
    ...
    constructor(
        input,
        {allowComments, allowNewLineMarkers, onComment, onToken, errors, warnings} = {},
    ) {
        /**
         * @private
         * @type {Token}
     ...
     */
    error(diagnostic, range = this.token) {
        this.errors.push(new Diagnostic(diagnostic, range));
    }

    /** @private */
    unexpectedEof() {
        this.error(DiagnosticCode.UnexpectedEof, {
     ...
     * @param {import('./Token.js').Range} range
     */
    warning(diagnostic, range = this.token) {
        this.warnings.push(new Diagnostic(diagnostic, range));
    }
    ...
    parseParams(closeParen) {
        ...
        while (this.nextToken() !== Token.NONE) {
            ...
            if (this.token.type === closeParen) {
                ...
                this.warning(DiagnosticCode.MismatchedCloseParen);
                break;
            }
        }
        return params;
    ...
    parseParenthesisedExpr() {
        ...
        if (token === Token.NONE) {
            ...
            this.error(DiagnosticCode.UnexpectedToken);
        } else {
            this.nextToken();
        }
        if (value === null) {
            this.error(DiagnosticCode.EmptyExpression, {
                start,
    ...
    parseUnaryExpression() {
        ...
        if (value === null) this.error(DiagnosticCode.UnexpectedToken);
        return {
    ...
    parse() {
        ...
        while (this.token !== Token.NONE) {
            if (
                ...
                this.error(DiagnosticCode.UnmatchedCloseParen);
                // Try to ignore the unmatched parenthesis
                this.nextToken();
            } else {
                this.error(DiagnosticCode.MissingSemicolon);
            }
            node = this.parseBinaryExpression(node);
        }
        return node;
 ...
 * @property {import('./Token.js').Position} start
 * @property {import('./Token.js').Position} end
 */

/**
 * @typedef {object} BinaryExpression
 * @property {'BinaryExpression'} kind
 * @property {string} operator The operator to apply to the
