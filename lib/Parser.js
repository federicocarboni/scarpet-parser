...
export class Parser {
     ...
     */
    nextToken() {
        if (this.token === TOKEN_NONE && this.prevToken !== TOKEN_NONE) return null;
    ...
    ensureValidLHS(node) {
        ...
        } else if (node.kind === 'Variable' && systemVariables.includes(node.name)) {
            this.diagnostic(
    ...
    getFunctionSignature(node, params, outerParams) {
        ...
        if (node.kind !== 'FunctionExpression') {
            this.diagnostic(DiagnosticSeverity.Error, DiagnosticCode.InvalidArrowOperator, node);
            return null;
        }
    ...
    parseFunction() {
        ...
        const params = this.parseParams(TokenType.CloseParen);
    ...
    parseBinaryExpression(lvalue, inMap = false) {
        ...
        if (rvalue === null && type !== TokenType.Semicolon) {
            this.diagnostic(DiagnosticSeverity.Error, DiagnosticCode.UnexpectedToken);
        }
        if (type === TokenType.Arrow && !inMap) {
            /** @type {Parameter[]} */
            const params = [];
            /** @type {OuterParameter[]} */
            const outerParams = [];
