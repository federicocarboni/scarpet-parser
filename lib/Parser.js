...
export class Parser {
    ...
    parseParams(closeParen, inMap = false) {
        ...
        while (this.nextToken() !== TOKEN_NONE) {
            let expr = this.parseExpression(void 0, inMap);
            if (expr !== void 0) params.push(expr);
            if (this.token === TOKEN_NONE) {
                this.unexpectedEof();
                break;
            }
    ...
    parsePrimaryExpression() {
        ...
        if (this.token.type.unaryOpPrecedence !== void 0) return this.parseUnaryExpression();
        return void 0;
    }
    ...
    parseBinaryExpression(lvalue, inMap = false) {
        ...
        if (rvalue === void 0 && token.type !== TokenType.Semicolon) {
            this.diagnostic(DiagnosticSeverity.Error, DiagnosticCode.UnexpectedToken);
        }
        ...
        if (token.type === TokenType.Arrow && !inMap) {
            ...
            if (resolved === void 0 || resolved.kind !== 'FunctionExpression') {
                this.diagnostic(
            ...
            if (variable !== void 0 && variable !== lvalue)
                this.diagnostic(
                    DiagnosticSeverity.Warning,
    ...
    parseExpression(precedence = 0, inMap = false) {
        ...
        while (this.token !== TOKEN_NONE) {
            if (this.token.type.binaryOpPrecedence === void 0) {
                if (
                    this.token.type !== TokenType.CloseParen &&
                    this.token.type !== TokenType.CloseBrack &&
                    this.token.type !== TokenType.CloseBrace &&
                    ...
                    this.diagnostic(DiagnosticSeverity.Error, DiagnosticCode.MissingSemicolon, {
                        start: this.prevToken.end,
                            ...
                            offset: this.prevToken.end.offset + 1,
                            line: this.prevToken.end.line,
                            character: this.prevToken.end.character + 1,
                        },
                    });
    ...
    parseMissingSemicolon(lvalue, inMap = false) {
        const rvalue = this.parseExpression(void 0, inMap);
        return {
            kind: 'BinaryExpression',
            operator: ';',
            lvalue,
    ...
    parse() {
        ...
        while (this.token !== TOKEN_NONE) {
            ...
            node = this.parseMissingSemicolon(node);
        }
        return node;
    }
}
