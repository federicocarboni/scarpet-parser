 ...
 * ```
 *
 * This parser tries its best to be consistent with Carpet Mod's behavior.
 *
 * For the sake of consistency certain edge cases may occasionally produce diagnostic errors or
 * warnings but execute fine in Carpet Mod.
 *
...
export class Parser {
    ...
    isVariable(node) {
        ...
        if (node.kind === 'BinaryExpression')
            return this.isVariable(node.lvalue);
    ...
    ensureValidLHS(node) {
        ...
        if (node.kind === 'Constant') {
            ...
            this.diagnostic(DiagnosticSeverity.Warning, DiagnosticCode.AssignToSystemVariable, node);
            return false;
        }
