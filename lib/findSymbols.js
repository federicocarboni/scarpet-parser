    ...
    for (const p of signature.params) {
        ...
        switch (param?.kind) {
            ...
            case "FunctionExpression": {
                ...
                if (variable !== undefined && variable.kind === "Variable") {
                    names.push({
                        kind: "outer",
                        variable,
                        value: undefined,
...
function findFunctionDeclarations(node, functions) {
    switch (node.kind) {
        case "BinaryExpression":
            ...
            if (node.rvalue !== undefined) findFunctionDeclarations(node.rvalue, functions);
            break;
        case "ParenthesisedExpression":
        case "UnaryExpression":
            if (node.value !== undefined) findFunctionDeclarations(node.value, functions);
            break;
